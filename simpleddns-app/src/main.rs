#![cfg_attr(not(debug_assertions), windows_subsystem = "windows")]  mod i18n;
 use eframe::egui;
...
// for Chinese support
    fonts.font_data.insert(         "NotoSansSC".to_owned(),         std::sync::Arc::new(egui::FontData::from_static(include_bytes!(             "../assets/fonts/NotoSansSC-Regular.otf"         ))),     );
     
// Put NotoSansSC as the highest priority
// for proportional fonts (UI text)

    fonts         .families         .entry(egui::FontFamily::Proportional)         .or_default()         .insert(0, "NotoSansSC".to_owned());
     
// Also
// for monospace fonts (Logs)

    fonts         .families         .entry(egui::FontFamily::Monospace)         .or_default()         .insert(0, "NotoSansSC".to_owned());
     ctx.set_fonts(fonts);
}
 ...
 fn pick_default_ipv6_interface() -> Option<String> {
    ...
    let mut names: Vec<String> = addrs         .into_iter()         .filter_map(|iface| match iface.addr.ip() {
 ...
 fn prefer_interface_ipv6_sources(config: &mut AppConfig) -> bool {
    let default_remote = vec![         "https://v6.ident.me".to_string(),         "https://api64.ipify.org".to_string(),     ];
...
// Try to attach to parent console (e.g., cmd/powershell)
...
// Keep the main thread alive and wait
// for shutdown signal
        ...
        tracing_subscriber::fmt()             .with_env_filter(EnvFilter::from_default_env().add_directive("info".parse().unwrap()))             .init();
         let options = eframe::NativeOptions {
            viewport: egui::ViewportBuilder::default()                 .with_inner_size([850.0, 600.0])                 .with_min_inner_size([650.0, 450.0])                 .with_icon(load_icon()),             ..Default::default()         };
         eframe::run_native(             "SimpleDDNS",             options,             Box::new(move |cc| {
                setup_custom_fonts(&cc.egui_ctx);
                Ok(Box::new(DdnsApp::new(                     storage, config, statuses, logs, config_tx, stop_tx, rt,                 )))             }),         )?;
        Ok(())     }
}
 fn build_scheduler_config(config: &AppConfig, storage: &StorageManager) -> SchedulerConfig {
    
// Inject tokens from encrypted secrets into provider_config
// // for each profile if missing

    let mut profiles = config.profiles.clone();
    storage.inject_tokens_into_profiles(&mut profiles);
     SchedulerConfig {
        running: true,         profiles,         interval_minutes: config.check_interval_minutes,         ipv4_urls: config.default_ipv4_urls.clone(),         ipv6_urls: config.default_ipv6_urls.clone(),     }
}
    ...
    let providers = build_providers();
     tokio::spawn(async move {
        ...
        loop {
            ...
            if cfg.running {
                scheduler                     .run_once(&cfg.profiles, &providers, &cfg.ipv4_urls, &cfg.ipv6_urls)                     .await;
            }
             let interval = std::time::Duration::from_secs(cfg.interval_minutes as u64 * 60);
            tokio::select! {
                _ = tokio::time::sleep(interval) => {},                 _ = stop_rx.changed() => {},                 _ = config_rx.changed() => {},             }
        }
    });
     stop_tx }
...
impl Default for ProfileEditor {
    fn default() -> Self {
        Self {
            is_open: false,             is_new: false,             profile: DdnsProfile::default(),             token: String::new(),             generic_url: String::new(),             generic_method: "GET".to_string(),             generic_body: String::new(),             generic_headers: String::new(),             save_error: None,         }
    }
}
 struct DdnsApp {
    storage: StorageManager,     config: AppConfig,     active_tab: usize,     editor: ProfileEditor,     statuses: SharedStatus,     logs: SharedLogs,     config_tx: watch::Sender<SchedulerConfig>,     _stop_tx: watch::Sender<bool>,     _rt: Runtime,     
// Cached copies
// for rendering (updated each frame from shared state)
    cached_statuses: HashMap<String, simpleddns_core::models::ProfileStatus>,     cached_logs: Vec<String>,     global_running: bool,     
// Settings editor
    new_ipv4_url: String,     new_ipv6_url: String, }
impl DdnsApp {
    fn new(         storage: StorageManager,         config: AppConfig,         statuses: SharedStatus,         logs: SharedLogs,         config_tx: watch::Sender<SchedulerConfig>,         stop_tx: watch::Sender<bool>,         rt: Runtime,     ) -> Self {
        Self {
            storage,             config,             active_tab: 0,             editor: ProfileEditor::default(),             statuses,             logs,             config_tx,             _stop_tx: stop_tx,             _rt: rt,             cached_statuses: HashMap::new(),             cached_logs: Vec::new(),             global_running: true,             new_ipv4_url: String::new(),             new_ipv6_url: String::new(),         }
    }
     fn notify_scheduler(&self) {
        let mut profiles = self.config.profiles.clone();
        self.storage.inject_tokens_into_profiles(&mut profiles);
         let sched_cfg = SchedulerConfig {
            running: self.global_running,             profiles,             interval_minutes: self.config.check_interval_minutes,             ipv4_urls: self.config.default_ipv4_urls.clone(),             ipv6_urls: self.config.default_ipv6_urls.clone(),         };
        if let Err(e) = self.config_tx.send(sched_cfg) {
            tracing::error!("Failed to notify scheduler: {}", e);
        }
    }
...
// // for live status updates
         ...
         egui::SidePanel::left("nav_panel")             .min_width(120.0)             .show(ctx, |ui| {
                ui.heading("SimpleDDNS");
                ui.add_space(10.0);
                 if ui                     .selectable_label(self.active_tab == 0, i18n::I18n::t(lang, "tab_profiles"))                     .clicked()                 {
                    self.active_tab = 0;
                }
                if ui                     .selectable_label(self.active_tab == 1, i18n::I18n::t(lang, "tab_settings"))                     .clicked()                 {
                    self.active_tab = 1;
                }
                 ui.add_space(20.0);
                ui.separator();
...
egui::TopBottomPanel::bottom("log_panel")             .min_height(100.0)             .max_height(200.0)             .resizable(true)             .show(ctx, |ui| {
                ui.horizontal(|ui| {
...
ui.horizontal(|ui| {
                        ...
                        if ui.checkbox(&mut enabled, "").changed() {
                         ...
                         let status_icon = match status {
                            Some(s) if s.status_message == "OK" => "OK",                             Some(_) => "Running",                             None => "Unknown",                         };
...
let plaintext_token = profile                                     .provider_config                                     .get("api_token")                                     .and_then(|v| v.as_str())                                     .map(|s| s.to_string());
...
let cfg = &profile.provider_config;
                                self.editor.generic_url = cfg                                     .get("url")                                     .and_then(|v| v.as_str())                                     .unwrap_or("")                                     .to_string();
                                self.editor.generic_method = cfg                                     .get("method")                                     .and_then(|v| v.as_str())                                     .unwrap_or("GET")                                     .to_string();
                                self.editor.generic_body = cfg                                     .get("body")                                     .and_then(|v| v.as_str())                                     .unwrap_or("")                                     .to_string();
                                self.editor.generic_headers = cfg                                     .get("headers")                                     .map(|v| serde_json::to_string_pretty(v).unwrap_or_default())                                     .unwrap_or_default();
                            }
                        });
                    });
...
ui.horizontal(|ui| {
                        ui.label(format!(                             "{}
{}",                             i18n::I18n::t(lang, "provider"),                             profile.provider_type                         ));
                        ui.separator();
...
// if error
                     if let Some(s) = status {
                        if s.status_message != "OK" {
                            ui.colored_label(                                 egui::Color32::from_rgb(255, 100, 100),                                 &s.status_message,                             );
                        }
                    }
                });
                ui.add_space(4.0);
     ...
     fn render_settings(&mut self, ui: &mut egui::Ui, lang: i18n::Language) {
        ...
        ui.separator();
         egui::ScrollArea::vertical()             .id_salt("settings_main_scroll")             .show(ui, |ui| {
                let mut changed = false;
                 ui.horizontal(|ui| {
                    ui.label(i18n::I18n::t(lang, "language"));
                    let mut current_lang = i18n::Language::from_str(&self.config.language);
                    egui::ComboBox::from_id_salt("language_select")                         .selected_text(match current_lang {
                            i18n::Language::En => i18n::I18n::t(lang, "lang_en"),                             i18n::Language::Zh => i18n::I18n::t(lang, "lang_zh"),                         })                         .show_ui(ui, |ui| {
         ...
         ui.horizontal(|ui| {
            ...
            if ui                 .add(egui::DragValue::new(&mut self.config.check_interval_minutes).range(1..=1440))                 .changed()             {
                changed = true;
            }
        });
...
if let Ok(exe) = std::env::current_exe() {
                if let Ok(launcher) = auto_launch::AutoLaunchBuilder::new()                     .set_app_name("SimpleDDNS")                     .set_app_path(exe.to_str().unwrap_or(""))                     .build()                 {
                    if run_on_startup {
        ...
        ui.heading(i18n::I18n::t(lang, "settings_interfaces"));
         egui::ScrollArea::vertical()             .id_salt("interfaces_scroll")             .max_height(150.0)             .show(ui, |ui| {
...
({})", name, first_ip));
        ...
        let mut ipv4_move_up = None;
        let mut ipv4_move_down = None;
        let ipv4_len = self.config.default_ipv4_urls.len();
        for (i, url) in self.config.default_ipv4_urls.iter().enumerate() {
            ui.horizontal(|ui| {
                ui.label(url);
                if i > 0 && ui.small_button("Move").clicked() {
                    ipv4_move_up = Some(i);
                }
                if i + 1 < ipv4_len && ui.small_button("Move").clicked() {
                    ipv4_move_down = Some(i);
                }
...
else if let Some(i) = ipv4_move_up {
            self.config.default_ipv4_urls.swap(i, i - 1);
            changed = true;
        }
else if let Some(i) = ipv4_move_down {
            self.config.default_ipv4_urls.swap(i, i + 1);
            changed = true;
        }
        ui.horizontal(|ui| {
            ...
            if ui.button(i18n::I18n::t(lang, "btn_add")).clicked() {
                self.config                     .default_ipv4_urls                     .push(self.new_ipv4_url.clone());
        ...
        for (i, url) in self.config.default_ipv6_urls.iter().enumerate() {
            ui.horizontal(|ui| {
                ...
                if i + 1 < ipv6_len && ui.small_button("Move").clicked() {
        ...
        ui.horizontal(|ui| {
            ...
            if ui.button(i18n::I18n::t(lang, "btn_add")).clicked() {
                self.config                     .default_ipv6_urls                     .push(self.new_ipv6_url.clone());
                ...
                ui.heading(i18n::I18n::t(lang, "backup_restore"));
                 ui.horizontal(|ui| {
                    if ui.button(i18n::I18n::t(lang, "btn_export")).clicked() {
                        if let Some(path) = rfd::FileDialog::new()                             .add_filter("JSON", &["json"])                             .set_file_name("simpleddns-backup.json")                             .save_file()                         {
     ...
     fn render_editor_modal(&mut self, ctx: &egui::Context) {
        ...
        if !is_open {
         ...
         egui::Window::new(if self.editor.is_new {
            ...
            i18n::I18n::t(lang, "edit_profile")         })         .open(&mut is_open)         .collapsible(false)         .resizable(true)         .min_width(400.0)         .show(ctx, |ui| {
            egui::Grid::new("profile_editor_grid")                 .num_columns(2)                 .spacing([10.0, 6.0])                 .show(ui, |ui| {
                    ...
                    egui::ComboBox::from_id_salt("provider_type")                         .selected_text(&self.editor.profile.provider_type)                         .show_ui(ui, |ui| {
                            ui.selectable_value(                                 &mut self.editor.profile.provider_type,                                 "cloudflare".to_string(),                                 "Cloudflare",                             );
                            ui.selectable_value(                                 &mut self.editor.profile.provider_type,                                 "generic".to_string(),                                 "Generic HTTP",                             );
                        });
                    ui.end_row();
                     ui.label("IPv4:");
...
if self.editor.profile.provider_type == "cloudflare" {
                ...
                egui::Grid::new("cf_config_grid")                     .num_columns(2)                     .spacing([10.0, 6.0])                     .show(ui, |ui| {
                        ui.label(i18n::I18n::t(lang, "api_token"));
                        ui.add(egui::TextEdit::singleline(&mut self.editor.token).password(true));
                        ui.end_row();
                         let mut zone_name = self                             .editor                             .profile                             .provider_config                             .get("zone_name")                             .and_then(|v| v.as_str())                             .unwrap_or("")                             .to_string();
                        ...
                        if ui.text_edit_singleline(&mut zone_name).changed() {
                            if let Some(obj) = self.editor.profile.provider_config.as_object_mut() {
                                obj.insert(                                     "zone_name".to_string(),                                     serde_json::Value::String(zone_name),                                 );
                            }
                        }
                        ui.end_row();
                    });
                ...
                egui::Grid::new("generic_config_grid")                     .num_columns(2)                     .spacing([10.0, 6.0])                     .show(ui, |ui| {
                        ...
                        egui::ComboBox::from_id_salt("http_method")                             .selected_text(&self.editor.generic_method)                             .show_ui(ui, |ui| {
                                ui.selectable_value(                                     &mut self.editor.generic_method,                                     "GET".to_string(),                                     "GET",                                 );
                                ui.selectable_value(                                     &mut self.editor.generic_method,                                     "POST".to_string(),                                     "POST",                                 );
                                ui.selectable_value(                                     &mut self.editor.generic_method,                                     "PUT".to_string(),                                     "PUT",                                 );
                                ui.selectable_value(                                     &mut self.editor.generic_method,                                     "PATCH".to_string(),                                     "PATCH",                                 );
                                ui.selectable_value(                                     &mut self.editor.generic_method,                                     "DELETE".to_string(),                                     "DELETE",                                 );
                            });
                        ui.end_row();
                    });
                 if self.editor.generic_method != "GET" {
                    ui.label(i18n::I18n::t(lang, "body_template"));
                    ui.add(                         egui::TextEdit::multiline(&mut self.editor.generic_body)                             .desired_rows(3)                             .code_editor(),                     );
                }
                 ui.label(i18n::I18n::t(lang, "custom_headers"));
                ui.add(                     egui::TextEdit::multiline(&mut self.editor.generic_headers)                         .desired_rows(3)                         .code_editor(),                 );
             ...
             ui.horizontal(|ui| {
                if ui.button(i18n::I18n::t(lang, "btn_save")).clicked() {
                    tracing::debug!("Save button clicked");
                    
// Reset error
self.editor.save_error = None;
                     
// Basic validation
if self.editor.profile.name.trim().is_empty() {
                        self.editor.save_error = Some("Name cannot be empty".to_string());
                        return;
                    }
...
if let Some(obj) =                                         self.editor.profile.provider_config.as_object_mut()                                     {
                                ...
                                Err(e) => {
                                    tracing::error!("Failed to save encrypted token: {}", e);
                                    self.editor.save_error = Some(format!("Encryption error: {}", e));
                                    return;
...
// Do not fallback to plaintext!
                        ...
                        obj.insert(                             "url".to_string(),                             serde_json::Value::String(self.editor.generic_url.clone()),                         );
                        obj.insert(                             "method".to_string(),                             serde_json::Value::String(self.editor.generic_method.clone()),                         );
                        if !self.editor.generic_body.is_empty() {
                            obj.insert(                                 "body".to_string(),                                 serde_json::Value::String(self.editor.generic_body.clone()),                             );
                        }
                        if !self.editor.generic_headers.is_empty() {
                            if let Ok(headers) = serde_json::from_str::<serde_json::Value>(                                 &self.editor.generic_headers,                             ) {
...
else if let Some(p) = self                         .config                         .profiles                         .iter_mut()                         .find(|p| p.id == self.editor.profile.id)                     {
                        *p = self.editor.profile.clone();
                    }
                    tracing::debug!("Saving config to file...");
