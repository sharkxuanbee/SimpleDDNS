...
simpleddns-app/          # GUI 入口 + 后台调度集成
simpleddns-core/         # 模型、Provider trait、IP 探测、调度器
simpleddns-providers/    # Cloudflare + Generic HTTP 实现
simpleddns-storage/      # JSON 持久化 + AES-GCM 加密存储
```
...
生成的可执行文件位于 `target/release/simpleddns-app`（Linux/macOS）或 `target/release/simpleddns-app.exe`（Windows）。
...
在 `simpleddns-providers` 中实现 `simpleddns_core::provider::DdnsProvider` trait，然后在 `simpleddns-app/src/main.rs` 的 `build_providers()` 中注册即可。
